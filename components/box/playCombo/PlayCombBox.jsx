  ...
  const playPunch = async (e, ciclo1) => {
    ...
    switch (e.punch) {
      ...
      case 'Cross':
        ...
        break;
      case 'Crochet L':
        soundPath = 'CrochetL.m4a';
        break;
      case 'Crochet R':
        soundPath = 'CrochetR.m4a';
        break;
      case 'Uppercut L':
        soundPath = 'UppercodeL.m4a';
        break;
      case 'Uppercut R':
        soundPath = 'UppercodeR.m4a';
        break;
      case 'Gancho L':
        soundPath = 'GanchoL.m4a';
        break;
      case 'Gancho R':
        soundPath = 'GanchoR.m4a';
        break;
      case 'Jav Body':
        soundPath = 'JavB.m4a';
        break;
      case 'Cross Body':
        soundPath = 'CrossB.m4a';
        break;
      default:
        return;
    }
    ...
    let duration = parseInt(e.duration, 10);
    ...
    if (isNaN(duration)) {
      ...
      try {
        ...
        await AudioModule.playAudio(soundPath, duration)
    ...
    if (isNaN(duration)) {
      ...
      try {
        ...
        await AudioModule.playAudio(soundPath, duration)
            ...
            await AudioModule.reanudarAudio(soundPath, duration, playTime)
          }
        const updatedConvinacion = [...combo.current.convinacion];
        updatedConvinacion.splice(0, 1);
        combo.current = {
    ...
    <View>
      ...
      <View style={styles.buttonContainer}>
       ...
       try {
        ...
        <Icon name="replay" size={50} color="#332b52" />
      </TouchableOpacity>
      <TouchableOpacity  style={styles.button} onPress={async () => {
        if(pause){
        setPause(false)
       ...
       try {
        ...
        <Icon name={pause ?  'pause' : 'play-arrow'} size={50} color="#332b52" />
      </TouchableOpacity>
      </View>
...
export default PlayCombBox

const styles = StyleSheet.create({
  button: {padding: 10, borderWidth: 1, borderRadius: 10, marginBottom: 5, zIndex: 1, width: 80, height: 80, borderWidth: 5},
  element: {width: '100%', height: '100%', position: 'absolute', zIndex: 3},
  progress: {marginBottom: 40, marginTop: 40},
  buttonContainer: {display: 'flex', flexDirection: 'row', marginLeft: '25%', gap: 40},
})